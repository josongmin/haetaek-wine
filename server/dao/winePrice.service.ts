// dao/winePrice.dao.ts
import db from '../config/db.js';
import { ResultSetHeader, RowDataPacket } from 'mysql2/promise';

interface WinePriceFilters {
  wineIndex?: number | string;
  shopIndex?: number | string;
  writerIndex?: number | string;
...
interface UpdateWinePriceParams {
  priceId: number;
  showSpecial?: boolean;
  showDetail?: boolean;
  stockCount?: number;
...
export const getWinePriceListContainWineInfo = async (filters: WinePriceFilters): Promise<RowDataPacket[]> => {
...
export const changeReviewStatus = async (status: number, winePriceIndex: number, point: number | null = null): Promise<number> => {
  const setClauses: string[] = ['WPR_status = ?'];
...
export const changeWriter = async (winePriceIndex: number, writerIndex: number): Promise<number> => {
  const sql = `
    UPDATE WinePrice
    SET WPR_writerIndex = ?
    WHERE WPR_index = ?
  ...
  try {
    const [result] = await db.query<ResultSetHeader>(sql, [writerIndex, winePriceIndex]);
    return result.affectedRows;
  } catch (err) {
 ...
 * @param show           특가 페이지 노출 여부
 * @param winePriceIndex         WinePrice 테이블의 PK
 * @returns       변경된 행의 개수
 */
export const setShowInSpecialPricePage = async (show: boolean, winePriceIndex: number): Promise<number> => {
  const sql = `
    UPDATE WinePrice
...
export const setShowInWineDetailPage = async (show: boolean, winePriceIndex: number): Promise<number> => {
  const sql = `
    UPDATE WinePrice
...
export const setStockCount = async (stockCount: number, winePriceIndex: number): Promise<number> => {
  const sql = `
    UPDATE WinePrice
       SET WPR_stockCount = ?
     WHERE WPR_index = ?
...
export const setHasReceipt = async (hasReceipt: boolean, winePriceIndex: number): Promise<number> => {
  const sql = `
    UPDATE WinePrice
       SET WPR_receipt = ?
     WHERE WPR_index = ?
  `;
  const params = [hasReceipt ? 1 : 0, winePriceIndex];
...
export const setNeededPointForShow = async (neededPoint: number, winePriceIndex: number): Promise<number> => {
  const sql = `
    UPDATE WinePrice
       SET WPR_point = ?
     WHERE WPR_index = ?
...
export async function updateWinePrice({
  ...
  hasReceipt,
  neededPoint,
}: UpdateWinePriceParams): Promise<number> {
  const sets: string[] = [];
  const params: any[] = [];
...
export const deleteWithRelatedData = async (priceIndex: number, wineIndex: number): Promise<number> => {
  ...
  `;
    const [deletePointResult] = await conn.query<ResultSetHeader>(sqlPoint, [
      priceIndex
    ]);
...
`;
    const [syncPointResult] = await conn.query<ResultSetHeader>(sqlSync, [
      priceIndex
    ]);
    ...
    const sqlPrice = `
      ...
      WHERE         WPR_index = ?
    `;
    const [deletePriceResult] = await conn.query<ResultSetHeader>(sqlPrice, [
      priceIndex
    ]);
  ...
  `;
    const [deleteWineResult] = await conn.query<ResultSetHeader>(sqlWine, [
      wineIndex
    ]);
  ...
  `;
    const [deleteReportResult] = await conn.query<ResultSetHeader>(sqlReport, [
      priceIndex
    ]);
