...
export const getList = async (searchText, sort = 1, status = null, type = null, onlyHeadShop = null, onlyBranch = null, lastRowIndex = null) => {
  // 1) loadMoreQuery
  let loadMoreQuery = '';
  if (lastRowIndex && lastRowIndex.length > 0) {
  ...
  //    (예시에서는 STATUS_PASS를 1이라고 가정합니다. 실제 코드와 동일한 상수를 사용할 것)
...
export const getUsedDiscountHistoryList = async (shopIndex, limit = 50) => {
  const sql = `
...
/*
* @param {string} shopIndex        가져올 샵 인덱스 (문자열)
* @param {string|null} lastRowIndex 마지막으로 불러온 WPR_index (이전 페이지가 있으면, 다음 페이지를 위해)
* @param {number|null} loadRowCount 가져올 최대 개수 (null 또는 0이 넘어오면 Java처럼 30개 제한)
* @returns {Promise<Array>}         WinePrice + WineShop + Wine 정보를 합친 배열
*/
export const getCommentHistoryList = async (
  ...
  let loadMoreQuery = " ";
  if (lastRowIndex != null && lastRowIndex.length > 0) {
    loadMoreQuery = " AND WP.WPR_index < " + lastRowIndex + " ";
  }

  // ── 2) Java 코드에서와 똑같이 statusQuery 작성 ──
  //    (ModelWinePrice.STATUS_PASS, STATUS_PASS_BEFORE_REVIEW를 그대로 붙임)
  const statusQuery =
    " AND (WP.WPR_status = " +
    PRICE_STATUS_PASS +
    " OR WP.WPR_status = " +
