...
❌ \`\`\`json 같은 코드 블록 사용 금지`;
    ...
    try {
      ...
      throw new Error(`Gemini API 네트워크 오류: ${err.message || String(fetchError)}`);
    }
    ...
    try {
      ...
      throw new Error(`Gemini API 응답 파싱 오류: ${err.message || String(jsonError)}`);
    }
    ...
    // jsonText를 try 블록 밖에서 선언하여 catch 블록에서도 접근 가능하도록
...
${JSON.stringify(wineObject, null, 2)}`;
      ...
      if (geminiResponse.ok) {
        ...
        if (geminiText) {
          ...
          try {
            const parsed = JSON.parse(jsonText) as { keyword_arrays?: unknown[] };
...
- JSON만 반환하고 다른 텍스트는 포함하지 마세요`;
    ...
    const successfulResults = results
      .filter((r): r is typeof r & { data: NonNullable<typeof r.data> } => r.success && !!r.data)
      .map(r => {
        const data = r.data;
...
export const searchSingleCandidateV3 = async (req: Request<{}, {}, SearchSingleCandidateV3Body>, res: Response): Promise<void> => {
  try {
    ...
    const serpData = await response.json() as SerpApiResponse;
    ...
    interface SearchResultItem {
      ...
      price: string | null;
      rich_snippet: unknown | null;
      date: null;
    }
    
    // 쇼핑 결과를 organic_results 형식으로 변환
    const convertedShoppingResults: SearchResultItem[] = shoppingResults.map((item: Record<string, unknown>, index: number) => ({
      position: organicResults.length + index + 1,
      title: (item.title || item.product_title || '') as string,
      link: (item.link || item.product_link || '') as string,
      displayed_link: (item.displayed_link || '') as string,
      snippet: (item.snippet || item.product_description || '') as string,
      thumbnail: (item.thumbnail || item.product_thumbnail || null) as string | null,
      price: (item.price || null) as string | null,
      rich_snippet: (item.rich_snippet || null) as unknown | null,
      date: null
    }));
    
    // organic_results와 shopping_results 합치기
    const allResults: Array<Record<string, unknown> | SearchResultItem> = [...organicResults, ...convertedShoppingResults];
    ...
    if (allResults.length > 0) {
      const firstResult = allResults[0] as Record<string, unknown>;
      console.log(`[searchSingleCandidateV3] 첫 번째 결과 구조:`, {
        ...
        snippet: typeof firstResult.snippet === 'string' ? firstResult.snippet.substring(0, 100) : undefined,
    ...
    if (serpData.knowledge_graph) {
      ...
      console.log(`[searchSingleCandidateV3] Knowledge Graph 정보:`, {
        title: kg.title,
        type: kg.type,
        description: kg.description?.substring(0, 100),
        image: kg.image,
        source: kg.source
      });
    }
    ...
    // 카드 결과 변환 및 점수 계산
    const cardResultsWithScore = allResults.map((result, index: number) => {
      const resultRecord = result as Record<string, unknown>;
      // 도메인 추출
      let domain: string | null = null;
      try {
        if (resultRecord.link) {
          domain = new URL(resultRecord.link as string).hostname.replace('www.', '');
        } else if (resultRecord.displayed_link) {
          domain = (resultRecord.displayed_link as string).replace('www.', '').split(' ›')[0];
        }
      } catch (e) {
        // URL 파싱 실패
      }
      ...
      if (resultRecord.snippet) {
        const priceMatch = (resultRecord.snippet as string).match(/(?:US\$|AU\$|€|£|¥|KRW|원)\s*[\d,]+(?:\.\d+)?(?:\s*-\s*(?:US\$|AU\$|€|£|¥|KRW|원)?\s*[\d,]+(?:\.\d+)?)?/);
        if (priceMatch) {
      ...
      if (!priceRange && (resultRecord.rich_snippet as Record<string, unknown>)?.top && ((resultRecord.rich_snippet as Record<string, unknown>).top as Record<string, unknown>)?.price) {
        priceRange = ((resultRecord.rich_snippet as Record<string, unknown>).top as Record<string, unknown>).price as string;
      }
      
      const thumbnailUrl = (resultRecord.thumbnail || resultRecord.thumbnail_url || ((resultRecord.rich_snippet as Record<string, unknown>)?.top as Record<string, unknown>)?.thumbnail || null) as string | null;
      
      // 위치 기반 관련성 점수 계산 (position이 낮을수록 관련성 높음)
      // position 1-3: 높음, 4-10: 중간, 11+: 낮음
      let relevanceScore = 0.5;
      let matchConfidence: 'high' | 'medium' | 'low' = 'medium';
      if (resultRecord.position) {
        const position = resultRecord.position as number;
        if (position <= 3) {
          relevanceScore = 0.9;
      ...
      if (!thumbnailUrl && index < 3) {
        console.log(`[searchSingleCandidateV3] 결과 ${index + 1} 썸네일 없음:`, {
          ...
          available_fields: Object.keys(resultRecord).filter(k => k.toLowerCase().includes('image') || k.toLowerCase().includes('thumb') || k.toLowerCase().includes('img'))
        });
      }
      
      // wine_name 정리: 플레이스홀더 제거 및 정리
      let wineName = (resultRecord.title as string) || candidate;
      ...
      const score = computeScore({
        title: resultRecord.title as string | undefined,
        snippet: resultRecord.snippet as string | undefined,
        url: resultRecord.link as string | undefined
      }, context);
      ...
      return {
        ...
        url: resultRecord.link || null,
        domain: domain || null,
        price_range: priceRange || null,
        description: resultRecord.snippet || null,
        date: resultRecord.date || null,
        position: resultRecord.position || null,
        sources: resultRecord.link ? [{
          site_name: siteName.charAt(0).toUpperCase() + siteName.slice(1),
          url: resultRecord.link as string,
        ...
        hasTrustedSite: true,
        rich_snippet: resultRecord.rich_snippet || null,
        score: score // 점수 추가
      };
    });
    ...
    const firstCard = cardResults[0] || {} as { wine_name?: string; thumbnail_url?: string | null; price_range?: string | null; description?: string | null };
...
- JSON 배열만 반환하고 다른 텍스트는 포함하지 마세요`;
    ...
    if (perplexityResult.status === 'fulfilled' && perplexityResult.value.ok) {
      try {
        const perplexityData = await perplexityResult.value.json() as { choices?: Array<{ message?: { content?: string } }> };
        const responseText = perplexityData.choices?.[0]?.message?.content;
        if (responseText) {
          let jsonText = responseText.trim();
          if (jsonText.includes('```')) {
          ...
          const winesArray: WineSearchResult[] = Array.isArray(parsed) ? parsed : ('wines' in parsed && Array.isArray(parsed.wines) ? parsed.wines : [parsed as WineSearchResult]);
    ...
    if (geminiResult.status === 'fulfilled' && geminiResult.value.ok) {
      try {
        const geminiData = await geminiResult.value.json() as { candidates?: Array<{ content?: { parts?: Array<{ text?: string }> } }> };
    ...
    if (openaiResult.status === 'fulfilled' && openaiResult.value.ok) {
      try {
        const openaiData = await openaiResult.value.json() as { choices?: Array<{ message?: { content?: string } }> };
    ...
    allWinesArrays.forEach(wine => {
      ...
      if (!winesMap.has(key)) {
        ...
        if (!existing) {
          // 기존 항목이 없으면 새로 추가
          winesMap.set(key, {
            candidate: wine.candidate || candidate,
