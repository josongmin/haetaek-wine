...
export function toLocalInputValue(rawValue: string | null | undefined): string {
  if (!rawValue) return "";
...
export function toLocalDisplay(rawValue: string | null | undefined): string {
  if (!rawValue) return "";
  const iso = rawValue.includes("T") ? rawValue : rawValue.replace(" ", "T"); // TZ 없음 = 로컬(KST)
  const d = new Date(iso);
  if (isNaN(d.getTime())) return "";
...
export function parseDateSafe(s: string | null | undefined): Date | null {
...
export function formatRelative(past: string | Date | null | undefined, now: Date = new Date()): string {
