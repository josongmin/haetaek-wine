// utils/pushMessage.js (새 파일 추천)
export function maskingPriceForLock(finalPrice) {
  ...
  export function buildHotDealMessage({ titleKR, vintage, finalPrice, needsLock }) {
    let msg = titleKR || '';
...
export function getLevelPushText(level) {
...
export function buildUserLevelChangedData({ userIndex, level }) {
  return {
    notificationType: "2",
