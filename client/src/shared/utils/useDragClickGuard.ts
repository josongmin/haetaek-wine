 ...
 */
export function useDragClickGuard(threshold: number = 5) {
  const sRef = useRef<DragState>({ x: 0, y: 0, dragging: false, downOnSelf: false });
  ...
  const guardClick = useCallback(<T extends MouseEvent>(handler?: (e: T) => void) => (e: T) => {
    const s = sRef.current;
    if (s.dragging) {
