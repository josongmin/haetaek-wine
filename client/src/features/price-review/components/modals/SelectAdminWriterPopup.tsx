// src/components/reviewPrice/SelectAdminWriterPopup.tsx
...
interface SelectAdminWriterPopupProps {
  isOpen: boolean;
  initialWriter?: any;
  priceIndex?: string | number;
  wineIndex?: string | number;
  onConfirm: (writer: any) => void;
  onCancel: () => void;
}
...
export default function SelectAdminWriterPopup({
  ...
  onConfirm,
  onCancel
}: SelectAdminWriterPopupProps) {
  const { user } = useContext(UserContext) as { user: any };
  
  const [selectedWriter, setSelectedWriter] = useState<any>(initialWriter);
