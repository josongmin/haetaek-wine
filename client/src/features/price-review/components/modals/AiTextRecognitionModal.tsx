  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (response && response.wines && response.wines.length > 0) {
        ...
        // 결과가 없을 때도 결과 영역을 표시하고 "결과 없음" 메시지 표시
        setWineSuggestions([]);
        setShowResults(true);
        toast.error('결과 없음');
      }
              ...
              {wineSuggestions.length > 0 ? (
                <div className="ai-wine-tabs">
                  {wineSuggestions.map((wine, index) => {
                    // candidates_en 배열의 첫 번째 값(가장 확률 높은 이름) 또는 original_ko 사용
                    const wineName = wine?.candidates_en?.[0] || wine?.original_ko || wine?.kr_name || wine?.en_name || `와인 ${index + 1}`;
                ...
                <div className="ai-no-results" style={{
                  padding: '20px',
                  textAlign: 'center',
                  color: '#666',
                  fontSize: '16px',
